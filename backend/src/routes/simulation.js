...
function parsePackets(source, type, fileContent = null) {
  let packets = [];

  if (type === 'text') {
    packets = source.split('\n')
      .map(line => line.trim())
    ...
    if (ext === '.txt') {
      ...
      try {
        ...
        const tempPath = path.join(__dirname, '../temp', `temp_${Date.now()}.js`);
...
router.post('/start', upload.single('packetFile'), async (req, res) => {
  try {
    ...
    if (inputType === 'text') {
      packets = parsePackets(textInput, 'text');
    } else if (inputType === 'file' && req.file) {
      const fileContent = fs.readFileSync(req.file.path, 'utf-8');
      packets = parsePackets(req.file.originalname, 'file', fileContent);
...
router.post('/preview', upload.single('packetFile'), (req, res) => {
  try {
    const { inputType, textInput } = req.body;
    let packets = [];
