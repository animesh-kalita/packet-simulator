...
const PacketStructureMap = ({ packetType, currentPosition, highlightedBytes = [] }) => {
      ...
      <Box p={3}>
        ...
        <Box mb={3} p={2} sx={{ bgcolor: 'action.selected', borderRadius: 1 }}>
          <Typography variant="h6" sx={{ textTransform: 'capitalize' }}>
            {Object.keys(PACKET_TYPES).find(key => PACKET_TYPES[key] === packetType) || 'Unknown'} Packet
          </Typography>
        </Box>
        ...
        <Box sx={{ overflowX: 'auto' }}>
          <Box display="flex" flexWrap="wrap" gap={1}>
            {fieldsWithRanges.map((field, index) => {
              ...
              const tooltip = getFieldTooltip(field, field.startByte, field.endByte);
              ...
              return (
                <Tooltip key={index} title={tooltip} placement="top">
                  <Box sx={(t) => ({
                    ...
                    fontWeight: 'medium',
                    color: color === theme.palette.action.hover ? t.palette.text.secondary : t.palette.common.white,
                    textAlign: 'center',
                    padding: 0.5
                  })}>
                    {field.name}
                    <Typography variant="caption" sx={{ display: 'block', mt: 1, fontSize: 9 }}>
        ...
        <Box mt={3}>
          ...
          <Box sx={{ maxHeight: 200, overflowY: 'auto' }}>
            {fieldsWithRanges.map((field, index) => (
              <Box key={index} sx={{ mb: 2, p: 1.5, bgcolor: getFieldColor(field.startByte, field.endByte) === theme.palette.action.hover ? 'action.hover' : 'background.paper', borderRadius: 1 }}>
                <Typography variant="h6" sx={{ mb: 1 }}>
                  {field.name}
                  <Chip label={`${field.startByte}-${field.endByte}`} size="small" sx={{ ml: 1 }} />
                </Typography>
                ...
                <Box mt={1.5} px={2}>
                  ...
                  {field.startByte <= currentPosition && field.endByte >= currentPosition && (
        ...
        <Box mt={3} p={2} sx={{ bgcolor: 'action.selected', borderRadius: 1 }}>
          ...
          <Box display="flex" flexWrap="wrap" gap={2}>
            <Box display="flex" alignItems="center" gap={1}>
              <Box sx={{ width: 20, height: 20, backgroundColor: 'success.light', borderRadius: 2 }} />
              <Typography variant="body2">Consumed (Parsed)</Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1}>
              <Box sx={{ width: 20, height: 20, backgroundColor: 'primary.light', borderRadius: 2 }} />
              <Typography variant="body2">Currently Processing</Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1}>
              <Box sx={{ width: 20, height: 20, backgroundColor: 'action.hover', borderRadius: 2 }} />
              <Typography variant="body2">Remaining (Not Parsed)</Typography>
            </Box>
            <Box display="flex" alignItems="center" gap={1}>
              <Box sx={{ width: 20, height: 20, backgroundColor: 'warning.light', borderRadius: 2 }} />
