...
export const classifyPacket = (packetHex) => {
  ...
  try {
    ...
    let imei = '';
    if (imeiHex.length === 16) { // 8 bytes = 16 hex chars
    ...
    const packetTypeName = Object.keys(PACKET_TYPES).find(
      key => PACKET_TYPES[key] === typeByte
    ) || `UNKNOWN_0x${typeByte.toString(16)}`;
    
    // Determine header type
    const headerTypeName = Object.keys(HEADERS).find(
    ...
    if ([HEADERS.DEFAULT, HEADERS.ALT_1, HEADERS.ALT_2, HEADERS.PIONEER].includes(header)) {
      confidence += 0.3;
    }
    ...
    const details = `Header: 0x${header.toString(16).toUpperCase()} (${headerTypeName}), ` +
    ...
    return {
      ...
      details: details,
      isKnown: Object.values(PACKET_TYPES).includes(typeByte) && 
               [HEADERS.DEFAULT, HEADERS.ALT_1, HEADERS.ALT_2, HEADERS.PIONEER].includes(header)
    };
  } catch (error) {
    return {
      type: null,
