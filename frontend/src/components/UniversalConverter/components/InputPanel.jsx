...
export default function InputPanel({
  ...
  const validation = input
    ? validateFormat(input, formatOverride || format)
    ...
    <Paper variant="outlined" sx={{ p: 2.5, mb: 2 }}>
      <Stack spacing={2}>
        <Stack
          ...
          useFlexGap
        >
          <Box sx={{ fontWeight: 600, fontSize: "0.95rem", mr: 1 }}>Input</Box>
          {detection ? (
        ...
        {showSettings && (
          ...
          >
            ...
            >
              ...
              <ToggleButton value={ENDIANNESS.LITTLE}>
                Little Endian
              </ToggleButton>
            </ToggleButtonGroup>
        ...
        {input && bytes && bytes.length > 0 && (
          <Alert severity="info" sx={{ py: 0.5, fontSize: "0.85rem" }}>
            <strong>{bytes.length}</strong> byte{bytes.length !== 1 ? "s" : ""}{" "}
            detected
