...
export function binaryToBytes(binStr) {
  ...
  if (clean.length === 0) return new Uint8Array(0);
  const padded = clean.length % 8 === 0 ? clean : clean.padStart(Math.ceil(clean.length / 8) * 8, '0');
...
export function bytesToASCII(bytes) {
  return bytes.map(b => (b >= 32 && b <= 126) || b === 10 || b === 13 ? String.fromCharCode(b) : '.').join('');
}
...
export function bytesToByteArray(bytes) {
  const hex = Array.from(bytes).map(b => '0x' + b.toString(16).padStart(2, '0').toUpperCase());
  return '[' + hex.join(', ') + ']';
}
